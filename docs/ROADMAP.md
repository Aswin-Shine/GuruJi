...
| 4 | Outbound WhatsApp delivery works | **Code done, unverified live** — send path and async ack implemented and tested against a mocked Cloud API. No Meta app, number or token exists yet; IMPLEMENTATION.md §7 is the checklist that turns this into **Done** |
...
2. **Stand up the Meta side and send one real message.** Developer app, test number, System User token, webhook subscription — IMPLEMENTATION.md §7. The first reply that lands on a verified recipient's phone closes criterion 4 and unblocks 3, 6, 7 and 8.
...
**Channel**
- Real OTP delivery over WhatsApp, replacing the dev bypass. Needs a Meta-approved *authentication* message template (a business-initiated message outside the 24-hour window cannot be free text), an OTP table with hashing and expiry, and per-number lockout. The sender module already exists; the template and the identity work do not.
- Read receipts and a typing indicator on WhatsApp. One extra Cloud API call per message; add the moment a pilot student says the bot "didn't see" their message during the 4–8 s wait.
- Refresh-token rotation.

**Operations**
- A real queue for the webhook turn, **only if** the background-task approach measurably loses messages (container restarts mid-turn are the failure mode — see ARCHITECTURE.md §7).
